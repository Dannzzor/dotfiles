...
if !exists('g:ariake_allow_italics')
  let g:ariake_allow_italics = 0
endif
...
if has('gui_running') || &t_Co == 88 || &t_Co == 256
  ...
  fun <sid>X(group, fg, bg, attr)
    let l:attr = a:attr
    if g:ariake_allow_italics == 0 && l:attr ==? 'italic'
      let l:attr= 'none'
    endif
  ...
  call <sid>X('Constant',       s:hue_5,         '',          '')
  call <sid>X('String',         s:hue_4,         '',          '')
  call <sid>X('Character',      s:hue_4,         '',          '')
  call <sid>X('Number',         s:hue_1_2,         '',          '')
  call <sid>X('Boolean',        s:hue_1_2,         '',          '')
  call <sid>X('Float',          s:hue_1_2,         '',          '')
  call <sid>X('Identifier',     s:hue_2,         '',          'none')
  ...
  call <sid>X('Operator',       s:hue_6, '',          'none')
  call <sid>X('Keyword',        s:hue_5_2,         '',          '')
  ...
  call <sid>X('javascriptExport',       s:hue_1,   '', '')
  call <sid>X('javascriptOpSymbols',    s:hue_1,   '', '')
  call <sid>X('javascriptClassKeyword', s:hue_1,   '', '')
  call <sid>X('jsModules',              s:hue_1,   '', '')

  "hue_1_2  #D7A0EE
  call <sid>X('javaScriptReserved',     s:hue_1_2, '', '')
  call <sid>X('javaScriptNull',         s:hue_1_2, '', '')
  call <sid>X('javaScriptNumber',       s:hue_1_2, '', '')
  call <sid>X('jsNull',                 s:hue_1_2,   '', '')
  call <sid>X('jsNan',                  s:hue_1_2,   '', '')
  call <sid>X('jsUndefined',            s:hue_1_2,   '', '')

  "hue_2    #7C7CD9
  call <sid>X('javascriptFuncKeyword',  s:hue_2,   '', '')
  call <sid>X('javascriptImport',       s:hue_2,   '', '')
  call <sid>X('jsClassKeywords',        s:hue_2,   '', '')
  call <sid>X('jsDocParam',             s:hue_2,   '', '')
  call <sid>X('jsFunction',             s:hue_2,   '', '')
  call <sid>X('jsArrowFunction',        s:hue_2,   '', '')
  call <sid>X('javaScriptFunction',     s:hue_2,   '', '')
  call <sid>X('javaScriptIdentifier',   s:hue_2,   '', '')
  call <sid>X('javascriptArrowFunc',    s:hue_2,   '', '')
  call <sid>X('javascriptClassExtends', s:hue_2,   '', '')
  call <sid>X('javascriptDocNotation',  s:hue_2,   '', '')
  call <sid>X('javascriptDocParamName', s:hue_2,   '', '')
  call <sid>X('javascriptDocTags',      s:hue_2,   '', '')
  call <sid>X('jsStorageClass',         s:hue_2,   '', '')

  "hue_3    #7FA8D5
  call <sid>X('jsFuncCall',             s:hue_3,   '', '')
  call <sid>X('jsModuleWords',          s:hue_3,   '', '')
  call <sid>X('javascriptIdentifier',   s:hue_3,   '', '')
  call <sid>X('javascriptObjectLabel',  s:hue_3,  '', '')
  call <sid>X('javascriptPropertyName', s:hue_3,   '', '')
  call <sid>X('javascriptVariable',     s:hue_3,   '', '')
  call <sid>X('javaScriptRequire',      s:hue_3,   '', '')
  call <sid>X('jsVariableDef',          s:hue_3,   '', '')
  call <sid>X('jsDocTags',              s:hue_3,   '', '')
  call <sid>X('jsClassFuncName',        s:hue_3,   '', '')
  call <sid>X('jsClassMethodType',      s:hue_3,   '', '')
  call <sid>X('jsClassDefinition',      s:hue_3,   '', '')
  call <sid>X('jsClassProperty',        s:hue_3,   '', '')
  call <sid>X('jsClassValue',           s:hue_3,   '', '')

  "hue_4    #97EBE6
  call <sid>X('coffeeString',           s:hue_4,   '', '')
  call <sid>X('jsOperator',             s:hue_4,   '', '')
  call <sid>X('jsNoise',                s:hue_4,  '', '')
  call <sid>X('jsTemplateVar',          s:hue_4,   '', '')
  call <sid>X('jsObjectValue',          s:hue_4,   '', '')

  "hue_5    #df2f8b
  call <sid>X('jsReturn',               s:hue_5,   '', '')
  call <sid>X('jsThis',                 s:hue_5,   '', '')
  call <sid>X('jsSuper',                s:hue_5,   '', '')
  ...
  "hue_6    #F2F7FC
  call <sid>X('jsClassBraces',          s:hue_6,  '', '')

  "hue_6_2  #97ebe6
  call <sid>X('javaScriptBraces',       s:hue_6_2, '', '')
  call <sid>X('jsGlobalObjects',        s:hue_6_2, '', '')

  "mono_1   #abb2bf
  call <sid>X('jsObjectKey',            s:mono_1,   '', '')
...
"}}}
" Public API --------------------------------------------------------------{{{
function! ariake#highlight(group, fg, bg, attr)
